...
from tenpcs.layers.input.exp_family import ExpFamilyLayer, CategoricalLayer
...
class TensorizedPC(nn.Module):
    ...
    def forward(self, x, return_layer_outputs: Optional[bool] = False) -> Tensor:
        """Evaluate the circuit in a feed forward way.

        Args:
            x (Tensor): The input to the circuit, shape (*B, D, C).
            return_layer_outputs (bool): True to return results of every layer.
        Returns:
            Tensor: The output of the circuit, shape (*B, K).
        """
        if isinstance(self.input_layer, CategoricalLayer):
            index = x if x.dtype == torch.long else x.long()
            num_channels = self.input_layer.num_channels
            if num_channels == 1:
                x = self.input_layer.params().squeeze().transpose(1, 2)[
                    range(self.num_vars), index.squeeze()].unsqueeze(-1)
            else:
                x = self.input_layer.params().view(self.num_vars, -1, num_channels, self.input_layer.num_categories)[
                    torch.arange(self.num_vars)[:, None].repeat(x.size(0), 1), :,
                    torch.arange(num_channels)[None, :],
                    index.flatten(0, 1)].view(x.size(0), self.num_vars, num_channels, -1).sum(2).unsqueeze(-1)
        else:
            x = self.input_layer(x)  # shape (*B, D, K, P)
        x = self.scope_layer(x)  # shape (F, K, *B)
        layer_outputs = self._eval_layers(x)
        if return_layer_outputs:
            return layer_outputs
        else:
            return layer_outputs[-1].squeeze(dim=0).movedim(source=0, destination=-1)  # shape (K, *B) -> (*B, K)
